...
scene.fog = new THREE.FogExp2(0x07000a, 0.02);
...
const cO = new THREE.Color(0xFF6B00), cP = new THREE.Color(0xFF006E), cY = new THREE.Color(0xFFD700);
...
const tMat = new THREE.PointsMaterial({ size: 0.1, vertexColors: true, transparent: true, opacity: 0.65, blending: THREE.AdditiveBlending, sizeAttenuation: true });
...
const iMat = new THREE.PointsMaterial({ size: 0.05, vertexColors: true, transparent: true, opacity: 0.45, blending: THREE.AdditiveBlending });
const inner = new THREE.Points(iGeo, iMat);
scene.add(inner);
...
window.addEventListener('resize', () => {
  ...
  <main id="main-content">
    ...
    <section className="info-section" id="sec-features" data-atx-section>
      <div className="info-container md:hidden">
      ...
      <div className="hidden md:block">
        <ScrollReveal effect="fadeUp" className="info-container mb-6">
          <div className="section-tag mono center-tag"><span className="tag-dot"></span> 05 // CAPACIDADES CORE</div>
          <h2 className="section-title center-title">LA VENTAJA<br /><span className="grad-text">ATHERNIX</span></h2>
        </ScrollReveal>
        <HorizontalScrollSection scrollMultiplier={2.5}>
          {[
            { icon: '🛡️', title: 'Escudo Cuántico', desc: 'Algoritmos criptográficos post-cuánticos protegen cada paquete de datos que atraviesa la malla.' },
            { icon: '🌐', title: 'Malla Auto-Sanadora', desc: 'Re-enrutamiento autónomo cuando los nodos se desconectan. Se requiere cero intervención manual.' },
            { icon: '⚡', title: 'Computo en el Borde', desc: 'Procesa datos en el borde con latencia sub-milisegundo en todas las 47 regiones.' },
            { icon: '👁️', title: 'Nodos Fantasma', desc: 'Puntos finales señuelo redirigen el tráfico adversario a vacíos honeypot automáticamente.' },
            { icon: '🔗', title: 'Autenticación Cero', desc: 'Verifica identidad sin exponer credenciales. Prueba matemática, no confianza.' },
            { icon: '🕐', title: 'Sincronización Crono', desc: 'Sincronización de tiempo con precisión de nanosegundos en cada punto final de la malla global.' },
          ].map((f) => (
            <div key={f.title} className="feature-card glass-card hs-card liquid-glass-strong card-3d" style={{ minWidth: 'min(360px, 80vw)', padding: '2rem' }}>
    ...
    <section className="info-section cta-section" id="sec-cta">
      <div className="info-container anim-reveal" style={{"textAlign":"center"}}>
        ...
        <div style={{"display":"flex","gap":"16px","justifyContent":"center","flexWrap":"wrap"}}>
          <MagneticElement>
          <a href="/register" className="cta-btn"><span>DESPLIEGA AHORA</span></a>
          </MagneticElement>
          <MagneticElement>
          <a href="/explore" className="sec-btn">VER DOCS</a>
          </MagneticElement>
        </div>
      </div>
    </section>
