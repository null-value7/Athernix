  ...
  return (
    <div className="space-y-10">
      ...
      <div className="tch-stagger grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map(c => (
          <div key={c.label} className="tch-stagger-item tch-glass liquid-glass glass-card-interactive card-3d hover:glow-orange tch-stat">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={onNewClassroom} className="mono text-xs px-4 py-2 rounded-full" style={{ background: `linear-gradient(135deg,${C_ORANGE},${C_YELLOW})`, color: '#08000a', fontWeight: 700}}>
          + NUEVA CLASE
        </button>
        </MagneticElement>
        <MagneticElement strength={15}>
        ...
        <div className="tch-stagger grid grid-cols-1 md:grid-cols-3 gap-4">
          {classes.map((c) => (
            <div key={c.id} className="tch-stagger-item tch-glass liquid-glass glass-card-interactive card-3d hover:glow-pink tch-class-card" onClick={() => { onSelectClass(c.id); onGoSection('estudiantes') }}>
...
function MissionCard({ mission, subject, cls, onToggleStatus }) {
  ...
  return (
    <div className="tch-stagger-item tch-glass liquid-glass glass-card-interactive card-3d hover:glow-orange tch-mission">
      ...
      <div className="tch-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="mt-7 flex items-center justify-end gap-3">
          ...
          <button onClick={onSubmit} disabled={!draft.name.trim() || submitting}
            ...
            style={{
              background: draft.name.trim() ? `linear-gradient(135deg,${C_ORANGE},${C_YELLOW})` : 'rgba(255,255,255,.08)',
...
function StudentRow({ student, cls, isExpanded, onSelect }) {
  ...
  return (
    <div className={`tch-stagger-item tch-glass liquid-glass glass-card-interactive card-3d hover:glow-pink tch-student ${isExpanded ? 'tch-expanded' : ''}`} onClick={() => onSelect(student.id)}>
...
function MateriasSection({ subjects, classes, assignments, onToggle }) {
  return (
    ...
    <div className="tch-stagger grid grid-cols-1 md:grid-cols-2 gap-5">
      {subjects.map((s) => (
        <div key={s.id} className="tch-stagger-item tch-glass liquid-glass glass-card-interactive card-3d hover:glow-orange tch-subject">
...
export default function TeacherDashboardPage() {
  ...
  return (
    <div ref={containerRef} className="tch-root relative min-h-screen" style={{ paddingTop: '80px' }}>
      <ParallaxLayer speed={0.4} direction="up"><div className="tch-orb float-gentle" style={{ width: 560, height: 560, top: '-15%', right: '-10%', background: 'radial-gradient(circle,rgba(255,0,110,0.12) 0%,transparent 70%)' }} /></ParallaxLayer>
      <ParallaxLayer speed={0.6} direction="down"><div className="tch-orb float-medium" style={{ width: 420, height: 420, bottom: '0%', left: '-10%', background: 'radial-gradient(circle,rgba(0,229,160,0.08) 0%,transparent 70%)' }} /></ParallaxLayer>
        ...
        <section className="tch-hero p-6 md:p-10">
          ...
          <div className="relative z-10">
            ...
            <h1 className="tch-title font-black leading-none mb-4" style={{ fontFamily: F_DISPLAY, fontSize: 'clamp(2.2rem,5vw,3.6rem)', letterSpacing: '.01em', opacity: 0 }}>
              <TextSplitter text="PANEL DE CLASSROOM" splitBy="word" effect="rise" gradient={true} />
            </h1>
