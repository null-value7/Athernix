...
import AthernixScrollEngine from "@/components/ui/AthernixScrollEngine";
import AthernixVisualEnhancer from "@/components/ui/AthernixVisualEnhancer";
    ...
    >
      ...
      <body className="min-h-screen flex flex-col" style={{ backgroundColor: '#08000a', color: '#fff' }}>
        {/* ── Librerías CDN globales — usadas vía window.* en componentes
            como page.tsx (GSAP, ScrollTrigger, Lenis).── */}
        <Script
        ...
        <AthernixVisualEnhancer />
        <CustomCursor />
        <Navbar />
        <AthernixScrollEngine showProgress={false} showSectionCounter enableLenis>
        <main className="flex-grow pt-20 pb-16 relative" style={{ minHeight: 'calc(100vh - 250px)', zIndex: 1 }}>{children}</main>
        </AthernixScrollEngine>
        <Footer />
      </body>
    </html>
  );
