  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Athernix colors: #FF6B00 (orange), #FF006E (pink), #FFD700 (yellow)
        vec3 deepSpace  = vec3(0.0, 0.0, 0.05);
        vec3 gasColor1  = vec3(1.0, 0.42, 0.0);   // #FF6B00 orange
        vec3 gasColor2  = vec3(1.0, 0.0, 0.43);   // #FF006E pink
        vec3 gasColor3  = vec3(1.0, 0.84, 0.0);   // #FFD700 yellow
        vec3 color      = deepSpace;

        color = mix(color, gasColor1, smoothstep(0.4, 0.6, c1));
        color = mix(color, gasColor2, smoothstep(0.5, 0.7, c2) * 0.5);
        color = mix(color, gasColor3, smoothstep(0.6, 0.8, c1 * c2) * 0.3);
