...
const LINKS = [
  { href: "/ondilla3", label: "Inicio" },
  { href: "/ondilla3#recorrido", label: "El recorrido" },
];
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (!nav) return;
    const onScroll = () => nav.classList.toggle("ond-scrolled", window.scrollY > 24);
    onScroll();
  ...
  return (
    <nav ref={navRef} className="ond-nav flex items-center justify-between">
      <Link href="/ondilla3" className="group flex items-center gap-2" data-cursor-hover>
        ...
        <span className="font-display text-gradient-fire text-xl tracking-[0.1em]"><span className="notranslate" translate="no">ATHERNIX</span></span>
      </Link>
      ...
      <ul className="hidden items-center gap-7 font-mono-label text-[11px] text-white/70 md:flex">
        ...
        >
          <Link
            href="/ondilla3/materias"
            className={cn(
              "link-underline flex items-center gap-1 transition-colors hover:text-white",
              pathname.startsWith("/ondilla3/materias") && "is-active text-white"
            )}
          >
      ...
      <Link href="/ondilla3/materias" className="hidden md:block" data-cursor-hover>
      ...
      {mobileOpen && (
        <div className="absolute left-0 top-full mt-2 flex w-full flex-col gap-1 rounded-2xl border border-white/10 bg-[#140406]/95 p-3 backdrop-blur-xl md:hidden">
          {[...LINKS, { href: "/ondilla3/materias", label: "Materias" }].map((l) => (
            <Link
              key={l.href}
              href={l.href}
