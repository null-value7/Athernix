...
export default function MateriaCard({ materia, index }: { materia: Materia; index: number }) {
  return (
    ...
    >
      <Link href={`/ondilla3/materias/${materia.slug}`} data-cursor-hover className="block">
        <TiltCard className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] p-8 transition-colors duration-300 hover:border-white/25">
          ...
          <h2 className="font-display relative mt-2 text-3xl text-white sm:text-4xl">{protectBrands(materia.nombre)}</h2>
          <p className="mt-1 text-sm italic text-white/50">{protectBrands(materia.tagline)}</p>
          <p className="relative mt-4 max-w-sm text-sm text-white/60">{protectBrands(materia.descripcion)}</p>
