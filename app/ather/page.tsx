...
export default function AthernixitoPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        scene.fog = new THREE.FogExp2(0x020005, 0.015); // Deeper cinematic black-purple
        const camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.1, 500);
        camera.position.set(6, 2, 10);
        camera.lookAt(0, 1, 0);
        scene.add(new THREE.AmbientLight(0x110524, 1.5)); // More subtle ambient

        const mainLight = new THREE.DirectionalLight(0xffb700, 4); // Warmer cinematic main
        mainLight.position.set(5, 10, 5);
        mainLight.castShadow = true;
        scene.add(mainLight);

        const rimLight = new THREE.DirectionalLight(0xff006e, 2);
        rimLight.position.set(-5, 3, -3);
        scene.add(rimLight);

        const fillLight = new THREE.PointLight(0xff6b00, 4, 20);
        fillLight.position.set(0, 4, 3);
        scene.add(fillLight);

        const backLight = new THREE.PointLight(0xff006e, 3, 15);
        ...
        // Planet rings
        const ringGeo = new THREE.TorusGeometry(8, 0.8, 4, 80);
        ...
        function createFallbackChar() {
          ...
          [-1, 1].forEach(side => {
            const legGeo = new THREE.CapsuleGeometry(0.18, 0.9, 4, 8);
  ...
  return (
    <>
      ...
      <AwardWinning3D 
        containerId="ather-3d-bg"
        variant="sphere"
        colors={{ primary: '#FFD700', secondary: '#FF006E', tertiary: '#FF6B00' }}
        intensity={1.4}
        interactive={true}
      />
      ...
      <div id="scroll-driver">
        <div className="sticky-hud">
          <LiquidGlassCard 
            ...
            style={{"left":"5vw","top":"50%","transform":"translateY(-50%)"}}
            id="panel-0"
          >
            <ScrollReveal effect="fadeUp">
              <div className="panel-badge"><span className="badge-dot"></span>TEMPORADA 01 · ACTIVA</div>
              <h1 className="panel-title gs-title"><TextSplitter text="EL ATHERNIXITO DESPIERTA" effect="wave" gradient="true" /></h1>
          ...
          >
            <ScrollReveal effect="fadeUp" delay={0.1}>
              ...
              <h1 className="panel-title gs-title"><TextSplitter text="ENTRA AL ESPACIO" effect="wave" gradient="true" /></h1>
          ...
          >
            <ScrollReveal effect="fadeUp" delay={0.1}>
              ...
              <h1 className="panel-title gs-title"><TextSplitter text="DOMINA EL FUTURO" effect="wave" gradient="true" /></h1>
          ...
          >
            <ScrollReveal effect="fadeUp" delay={0.1}>
              ...
              <h1 className="panel-title gs-title"><TextSplitter text="POTENCIA ILIMITADA" effect="wave" gradient="true" /></h1>
